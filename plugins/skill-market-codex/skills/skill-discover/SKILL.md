---
name: skill-discover
description: "Browse or search the Codex Skill Market catalog without installing packages. Use to find available plugins or standalone skills, inspect catalog status, or resolve a stable package id."
---
# Skill Discover for Codex
...
Run `node "<plugin-root>/cli/bin/skill-market.mjs" discover [quoted-query] --adapter codex [--kind ...] [--status ...] [--latest|--offline] [--offset N] [--limit N] --json`. Omit the query to browse active entries; `--latest` and `--offline` are mutually exclusive; use `--adapter all` only when explicitly requested.
