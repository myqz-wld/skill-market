...
Create or update only PlantUML `.puml` files. Ask only for required missing drawing inputs and wait for the answer before drawing. Once inputs are available, inspect source evidence with Grok's terminal and file-reading tools, then edit `.puml` files with its file-editing tools. For new diagrams, use `/tmp/plantuml-diagrams/` unless the user explicitly specifies an output path.
...
2. Read source or context evidence with Grok's terminal or file-reading tools, then build an evidence map for major nodes, edges, call chains, and state transitions.
3. Create or update `.puml` files with Grok's file-editing tools using the evidence map and the file rules below.
4. Check `@startuml` / `@enduml` pairing; when PlantUML is installed, run `plantuml -syntax <file>.puml` through the terminal tool.
...
- If PlantUML CLI is unavailable, verify `@startuml` / `@enduml` pairing with the file-reading tool and report that strict syntax validation was not run.
