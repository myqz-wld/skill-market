...
Treat the repository root as the base for every path recorded in project-authored instructions, plans, reviews, changelogs, indexes, validation evidence, and archived LLM-facing materials. Record in-repository files as paths such as `src/...`, `ref/...`, or `.` for the repository root; do not persist the repository or worktree's absolute prefix, a home-directory path, or a username. Refer to external resources with a portable environment-variable reference when one exists; otherwise use a non-identifying logical label and describe its relationship without a machine-specific path.

Before finalizing an affected record, use `rg` transiently to search the relevant project text for the current repository, worktree, and home-directory prefixes. Replace every match or document the exact non-sensitive exception outside the record; do not archive the scan output.
