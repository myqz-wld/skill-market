...
Use this skill for one-shot inspection, repair, or setup. Inspect the repository first, preserve project-specific invariants, and choose an execution mode before editing. After setup, the project follows its own `CLAUDE.md` workflow SSOT and `UI_COPY_LANGUAGE.md` UI/CLI copy-language SSOT. Use `rg` through Grok's terminal tool for inspection and Grok's file-editing tools for manual edits.
...
Use only the needed templates from `assets/templates/`; create or update project files with Grok's file-editing tools, not shell redirection. Merge missing template sections into existing files instead of overwriting project-specific instructions.
