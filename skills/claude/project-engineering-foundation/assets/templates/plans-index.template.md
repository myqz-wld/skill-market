...
> Scope: final plan documents. Keep non-final plans in the current environment's plan workspace. If no stronger contract exists, use `<repo>/.ref/plans/`; add `.ref/` to `.gitignore` and do not store non-final plans here.
