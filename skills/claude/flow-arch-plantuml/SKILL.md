...
Use this skill to create or update PlantUML `.puml` diagrams when requested. Use `AskUserQuestion` for missing drawing inputs when available; inspect with Bash/Read and edit `.puml` files with file tools.
...
Wait for the user's inputs if required information is missing. Do not create or edit `.puml` files before the required inputs are available.
