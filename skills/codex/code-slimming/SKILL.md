...
Use this skill to reduce code size with evidence. Run a scan-only phase first, wait for candidate approval, then edit only approved items. Use `rg` / shell for inspection and `apply_patch` for manual edits.
