...
Use this skill for one-shot inspection, repair, or setup. Inspect the repository first, preserve project-specific invariants, and choose an execution mode before editing. After setup, the project follows its own `CLAUDE.md` workflow SSOT and `UI_COPY_LANGUAGE.md` UI/CLI copy-language SSOT. Use `rg` / shell for inspection and `apply_patch` for manual file edits.
