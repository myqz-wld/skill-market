...
Use this skill to decide whether a core workflow or architecture change needs a PlantUML diagram, then create, update, or archive the `.puml` source diagram after confirmation. Ask missing decisions in the response, end the turn, then inspect with `shell` and edit `.puml` files with `apply_patch` after the user confirms.
