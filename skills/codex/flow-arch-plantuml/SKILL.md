...
Use this skill to confirm whether a core workflow or architecture change needs a PlantUML diagram, create or update the `.puml` SSOT, and maintain the matching `ref/flows` or `ref/architecture` INDEX. Ask the confirmation questions in the response, end the turn, then inspect with `shell` and edit `.puml` / INDEX files with `apply_patch` after the user confirms.
...
| PlantUML CLI is unavailable | Verify `@startuml` / `@enduml` pairing with `shell`; report that strict syntax validation was not run. |
