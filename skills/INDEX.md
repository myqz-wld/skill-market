...
| claude | flow-arch-plantuml | 0.0.5 | skills/claude/flow-arch-plantuml | active | Use when the user asks for a flow/architecture diagram or a change affects a core workflow, state machine, protocol, process boundary, schema, permission boundary, or module architecture. Creates or updates PlantUML `.puml` diagrams only. |
| claude | parallel-tasks | 0.0.3 | skills/claude/parallel-tasks | active | Use when a task has 2+ independent tracks across modules, files, audits, research, tests, or implementation areas with disjoint write sets that parallel agents can run concurrently. |
...
| codex | flow-arch-plantuml | 0.0.5 | skills/codex/flow-arch-plantuml | active | Use when the user asks for a flow/architecture diagram or a change affects a core workflow, state machine, protocol, process boundary, schema, permission boundary, or module architecture. Creates or updates PlantUML `.puml` diagrams only. |
| codex | parallel-tasks | 0.0.3 | skills/codex/parallel-tasks | active | Use when a task has 2+ independent tracks across modules, files, audits, research, tests, or implementation areas with disjoint write sets that parallel agents can run concurrently. |
